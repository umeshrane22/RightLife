...
class ProteinFragment : BaseFragment<FragmentProteinBinding>() {
    ...
    private val viewModel: ActiveBurnViewModelProtien by viewModels()
...
//        val date = parts.getOrNull(1) ?: ""
    ...
    private fun lineChartForSixMonths() {
        ...
        lineChart.post {
            ...
            val hMargin = 40f
            val vMargin = 20f
            if (viewModel.currentRange == RangeTypeChartsProtien.SIX_MONTHS) {
...
class ActiveBurnViewModelProtien : ViewModel() {
    val allDailySteps = mutableListOf<DailyStepProtien>()
    val goalSteps = 10000
    var currentRange = RangeTypeChartsProtien.SIX_MONTHS
    var startDate = Date()
    var endDate = Date()

    init {
        generateMockData()
        setInitialRange(RangeTypeChartsProtien.SIX_MONTHS)
    }
    ...
    private fun generateMockData() {
        ...
        while (current <= end) {
            ...
            allDailySteps.add(DailyStepProtien(current, randomSteps))
    ...
    val filteredData: List<DailyStepProtien>
        get() = allDailySteps.filter { it.date >= startDate && it.date <= endDate }

    val monthlyGroups: List<MonthGroupsProtien>
        get() = if (currentRange != RangeTypeChartsProtien.SIX_MONTHS || filteredData.isEmpty()) emptyList() else {
            val calendar = Calendar.getInstance()
            ...
            var results = mutableListOf<MonthGroupsProtien>()
            ...
            for ((_, days) in sorted) {
                ...
                results.add(MonthGroupsProtien(sortedDays, avg, diffString, isPositive, clampedStart, clampedEnd))
            }
            results
        }

    fun setInitialRange(range: RangeTypeChartsProtien) {
        currentRange = range
        val calendar = Calendar.getInstance()
        val now = Date()
        when (range) {
            RangeTypeChartsProtien.WEEK -> {
            ...
            RangeTypeChartsProtien.SIX_MONTHS -> {
                calendar.time = now
    ...
    fun goLeft() {
        ...
        calendar.time = startDate
        when (currentRange) {
            RangeTypeChartsProtien.WEEK -> calendar.add(Calendar.DAY_OF_YEAR, -7)
            RangeTypeChartsProtien.MONTH -> calendar.add(Calendar.MONTH, -1)
            RangeTypeChartsProtien.SIX_MONTHS -> calendar.add(Calendar.MONTH, -6)
        }
        startDate = calendar.time
...
data class DailyStepProtien(val date: Date, val steps: Int)
enum class RangeTypeChartsProtien { WEEK, MONTH, SIX_MONTHS }
data class MonthGroupsProtien(
    val days: List<DailyStepProtien>,
    val avgSteps: Int,
...
private fun List<DailyStepProtien>.averageSteps(): Number {
    return if (isEmpty()) 0.0 else sumOf { it.steps } / size
}

class CurvedBarChartRendererProtien(
    chart: BarChart,
