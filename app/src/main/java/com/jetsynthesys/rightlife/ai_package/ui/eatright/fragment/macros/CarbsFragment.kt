...
class CarbsFragment : BaseFragment<FragmentCarbsBinding>() {
    ...
    private val viewModel: ActiveBurnViewModelCarbs by viewModels()
...
//        val date = parts.getOrNull(1) ?: ""
    ...
    private fun lineChartForSixMonths() {
        ...
        lineChart.post {
            ...
            val hMargin = 40f
            val vMargin = 20f
            if (viewModel.currentRange == RangeTypeChartsCarbs.SIX_MONTHS) {
...
class ActiveBurnViewModelCarbs : ViewModel() {
    val allDailySteps = mutableListOf<DailyStepCarbs>()
    val goalSteps = 10000
    var currentRange = RangeTypeChartsCarbs.SIX_MONTHS
    var startDate = Date()
    var endDate = Date()

    init {
        generateMockData()
        setInitialRange(RangeTypeChartsCarbs.SIX_MONTHS)
    }
    ...
    private fun generateMockData() {
        ...
        while (current <= end) {
            ...
            allDailySteps.add(DailyStepCarbs(current, randomSteps))
    ...
    val filteredData: List<DailyStepCarbs>
        get() = allDailySteps.filter { it.date >= startDate && it.date <= endDate }

    val monthlyGroups: List<MonthGroupsCarbs>
        get() = if (currentRange != RangeTypeChartsCarbs.SIX_MONTHS || filteredData.isEmpty()) emptyList() else {
            val calendar = Calendar.getInstance()
            ...
            var results = mutableListOf<MonthGroupsCarbs>()
            ...
            for ((_, days) in sorted) {
                ...
                results.add(MonthGroupsCarbs(sortedDays, avg, diffString, isPositive, clampedStart, clampedEnd))
            }
            results
        }

    fun setInitialRange(range: RangeTypeChartsCarbs) {
        currentRange = range
        val calendar = Calendar.getInstance()
        val now = Date()
        when (range) {
            RangeTypeChartsCarbs.WEEK -> {
            ...
            RangeTypeChartsCarbs.SIX_MONTHS -> {
                calendar.time = now
    ...
    fun goLeft() {
        ...
        calendar.time = startDate
        when (currentRange) {
            RangeTypeChartsCarbs.WEEK -> calendar.add(Calendar.DAY_OF_YEAR, -7)
            RangeTypeChartsCarbs.MONTH -> calendar.add(Calendar.MONTH, -1)
            RangeTypeChartsCarbs.SIX_MONTHS -> calendar.add(Calendar.MONTH, -6)
        }
        startDate = calendar.time
...
data class DailyStepCarbs(val date: Date, val steps: Int)
enum class RangeTypeChartsCarbs { WEEK, MONTH, SIX_MONTHS }
data class MonthGroupsCarbs(
    val days: List<DailyStepCarbs>,
    val avgSteps: Int,
...
private fun List<DailyStepCarbs>.averageSteps(): Number {
    return if (isEmpty()) 0.0 else sumOf { it.steps } / size
}

class CurvedBarChartRendererCarbs(
    chart: BarChart,
